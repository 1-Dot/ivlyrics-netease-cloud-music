...
(() => {
    ...
    const ADDON_INFO = {
        ...
        description: {
            ...
            'zh-CN': '从网易云音乐获取同步歌词和官方翻译歌词。默认使用 netease.happyking.top 这个公开 NeteaseCloudMusicApi 镜像；填写 CORS 代理后改为请求官方网易云接口。'
        },
        supports: {
            karaoke: false,
            synced: true,
    ...
    function getSetting(key, defaultValue) {
        ...
        let raw = null;
        try {
            raw = Spicetify?.LocalStorage?.get(`${STORAGE_KEY_PREFIX}${ADDON_ID}:${key}`);
        } catch {
            raw = null;
        }
    ...
    function setSetting(key, value) {
        ...
        try {
            Spicetify?.LocalStorage?.set(`${STORAGE_KEY_PREFIX}${ADDON_ID}:${key}`, JSON.stringify(value));
        } catch {
    ...
    async function searchNetease(info) {
        ...
        if (hasProxyUrl()) {
            ...
            if (isOfficialJsonUnusable(data)) {
                throw new Error('Official NetEase search response is unusable. Please check your CORS proxy output and exit IP.');
            }
            return {
                source: 'official',
    ...
    async function fetchLyrics(songId) {
        if (hasProxyUrl()) {
            ...
            const data = await fetchJson(officialUrl, { throughProxy: true });
            if (isOfficialJsonUnusable(data)) {
                throw new Error('Official NetEase lyric response is unusable. Please check your CORS proxy output and exit IP.');
            }
            return {
                source: 'official',
                data
        ...
        return function NeteaseLyricsSettings() {
            ...
            return React.createElement('div', { className: 'ai-addon-settings netease-settings' },
                ...
                React.createElement('div', { className: 'ai-addon-setting' },
                    React.createElement('label', null, 'CORS proxy URL (optional, recommended)'),
                    ...
                    React.createElement('small', null, 'Leave empty to use public mirrors. Fill this to request official music.163.com APIs through your proxy. Use {url} placeholder if your proxy needs it; otherwise the encoded URL is appended after the prefix.')
                ),
                React.createElement('div', { className: 'ai-addon-setting' },
                ...
                React.createElement('div', { className: 'ai-addon-setting' },
                    ...
                    React.createElement('small', null, 'This intercepts ivLyrics translation requests only for this provider and only when translated lyrics were fetched.')
                ),
                React.createElement('div', { className: 'ai-addon-setting' },
                    React.createElement('label', null,
                        React.createElement('input', {
    ...
    const NeteaseLyricsAddon = {
        ...
        async getLyrics(info) {
            ...
            if (!synced.length) {
                ...
                if (useTranslation) {
                    const translatedRaw = lyricData?.ytlrc?.lyric || lyricData?.tlyric?.lyric || lyricData?.trans || '';
            ...
            result.netease = {
                ...
                source: hasProxyUrl() ? 'official-api-via-cors-proxy' : 'happyking-neteasecloudmusicapi-mirror',
