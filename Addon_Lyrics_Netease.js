...
(() => {
  ...
  const ADDON_INFO = {
    ...
    version: "0.1.4",
    description: {
      en: "Get synced lyrics and official translated lyrics from NetEase Cloud Music. Uses public NeteaseCloudMusicApi mirrors by default; configure a CORS proxy to request official NetEase APIs instead.",
      "zh-CN":
        "从网易云音乐获取同步歌词和官方翻译歌词。默认使用公开 NeteaseCloudMusicApi 镜像；填写 CORS 代理后改为请求官方网易云接口。",
    },
    supports: {
      karaoke: false,
      synced: true,
  ...
  function getSetting(key, defaultValue) {
    ...
    let raw = null;
    try {
      raw = Spicetify?.LocalStorage?.get(
        `${STORAGE_KEY_PREFIX}${ADDON_ID}:${key}`,
      );
    } catch {
      raw = null;
    }
  ...
  function setSetting(key, value) {
    ...
    try {
      ...
      // Settings are best-effort when the addon manager is unavailable.
    }
  }
  ...
  function detectRomanizationLanguage(text) {
    const mode = String(
      getSetting("local_phonetic_language", "auto") || "auto",
    );
  ...
  function textScore(source, candidate) {
    ...
    if (a.includes(b) || b.includes(a) || ac.includes(bc) || bc.includes(ac))
      return 0.82;
  ...
  function scoreSong(info, song) {
    ...
    const total = Math.round(
      title * 52 + artists * 33 + album * 8 + duration * 7,
    );
    return {
      total,
      title,
  ...
  function pickPublicNodeHost() {
    return PUBLIC_NODE_API_HOSTS[
      Math.floor(Math.random() * PUBLIC_NODE_API_HOSTS.length)
    ];
  }
  ...
  function isOfficialJsonUnusable(data) {
    if (!data || typeof data !== "object") return true;
    if (data.code && Number(data.code) !== 200) return true;

    // Some failed official requests return encrypted/unexpected payloads instead of normal JSON fields.
    if (typeof data === "string") return true;
    if (data.abroad || data.needLogin) return true;
  ...
  function parseLrc(raw) {
    ...
    return lines
      ...
      .filter((line, index, arr) => {
        ...
        return (
          !previous ||
          previous.startTime !== line.startTime ||
  ...
  function mergeTranslatedLines(baseLines, translatedLines) {
    ...
    return baseLines.map((line) => {
      ...
      for (const translated of translatedLines) {
        const diff = Math.abs(
          Number(translated.startTime) - Number(line.startTime),
        );
        if (diff < bestDiff) {
          best = translated;
          bestDiff = diff;
      ...
      const translatedText =
        best && bestDiff <= TRANSLATION_TIME_TOLERANCE_MS ? best.text : "";
  ...
  function persistTranslations(trackId, translations) {
    ...
    try {
      Spicetify?.LocalStorage?.set(
        `${TRANSLATION_CACHE_PREFIX}${trackId}`,
  ...
  function rememberTranslations(trackIds, lines) {
    ...
    const translations = (lines || []).map(
      (line) => line.text2 || line.translation || line.translationText || "",
    );
  ...
  function readPersistedPhonetic(trackId, textHash) {
    ...
    try {
      const parsed = JSON.parse(raw);
      if (
        parsed?.hash === textHash &&
        Array.isArray(parsed.phonetic) &&
        parsed.phonetic.some(Boolean)
      ) {
  ...
  function persistPhonetic(trackIds, textHash, phonetic) {
    ...
    for (const trackId of (Array.isArray(trackIds)
      ? trackIds
      : [trackIds]
    ).filter(Boolean)) {
      try {
        Spicetify?.LocalStorage?.set(
          `${PHONETIC_CACHE_PREFIX}${trackId}`,
          payload,
        );
      } catch {
        try {
  ...
  async function generateLocalPhoneticLines(text) {
    ...
    if (language === "ja") {
      return Promise.all(
        lines.map((line) =>
          translator.romajifyText(line || "", "romaji", "spaced"),
        ),
      );
    }
    if (language === "ko") {
      return Promise.all(
        lines.map((line) => translator.convertToRomaja(line || "", "romaja")),
      );
    }
    if (language === "zh") {
      return Promise.all(
        lines.map((line) =>
          translator.convertToPinyin(line || "", {
            toneType:
              getSetting("local_phonetic_pinyin_tone", "mark") || "mark",
            type: "string",
          }),
        ),
      );
  ...
  function installTranslatorGuard() {
    const guardEnabled =
      getSetting(
        "skip_ai_translation_when_netease_translation_exists",
        true,
      ) !== false;
    const localPhoneticEnabled =
      getSetting("enable_local_phonetic", true) !== false;
    if (!guardEnabled && !localPhoneticEnabled) return;

    if (
      !window.Translator ||
      typeof window.Translator.callGemini !== "function"
    ) {
    ...
    if (window.Translator.__ivLyricsNeteaseGuardVersion === ADDON_INFO.version)
      return;

    const originalCallGemini = window.Translator.callGemini.bind(
      window.Translator,
    );
    window.Translator.callGemini = async function guardedCallGemini(payload) {
      ...
      if (localPhoneticNow && wantsPhonetic && provider === ADDON_ID) {
        try {
          ...
          console.warn(
            "[NetEase Lyrics Addon] Local romanization failed:",
            error,
          );
          const allowAiFallback =
            getSetting("local_phonetic_fallback_to_ai", false) === true;
          if (!allowAiFallback) {
            const fallbackLines = String(payload?.text || "")
              .split(/\r?\n/)
              .map(() => "");
  ...
  function createSettingsUI() {
    ...
    return function NeteaseLyricsSettings() {
      const [searchLimit, setSearchLimit] = React.useState(
        String(getSetting("search_limit", 8)),
      );
      const [minScore, setMinScore] = React.useState(
        String(getSetting("min_score", 58)),
      );
      const [proxyUrl, setProxyUrl] = React.useState(
        String(getSetting("proxy_url", "") || ""),
      );
      const [enableTranslation, setEnableTranslation] = React.useState(
        getSetting("enable_netease_translation", true) !== false,
      );
      const [skipAiTranslation, setSkipAiTranslation] = React.useState(
        getSetting(
          "skip_ai_translation_when_netease_translation_exists",
          true,
        ) !== false,
      );
      const [enableLocalPhonetic, setEnableLocalPhonetic] = React.useState(
        getSetting("enable_local_phonetic", true) !== false,
      );
      const [localPhoneticLanguage, setLocalPhoneticLanguage] = React.useState(
        String(getSetting("local_phonetic_language", "auto") || "auto"),
      );
      const [localPhoneticFallbackToAi, setLocalPhoneticFallbackToAi] =
        React.useState(
          getSetting("local_phonetic_fallback_to_ai", false) === true,
        );
      ...
      return React.createElement(
        "div",
        { className: "ai-addon-settings netease-settings" },
        React.createElement(
          "div",
          {
            ...
            style: {
              padding: "12px",
              borderRadius: "8px",
              background: "rgba(224, 38, 50, 0.12)",
              border: "1px solid rgba(224, 38, 50, 0.38)",
            },
          },
          React.createElement("strong", null, "Lyrics source"),
          React.createElement(
            "p",
            { style: { margin: "6px 0 0" } },
            "Default: public NeteaseCloudMusicApi mirrors, similar to DualSubs. If you fill a CORS proxy URL below, this addon switches to official NetEase Cloud Music web APIs through your proxy.",
          ),
        ),
        React.createElement(
          "div",
        ...
        React.createElement(
          ...
          React.createElement("input", {
            ...
            value: searchLimit,
            onChange: (e) =>
              saveNumber("search_limit", e.target.value, 1, 20, setSearchLimit),
          }),
          React.createElement(
            "small",
            null,
            "How many NetEase candidates to inspect for each Spotify track.",
          ),
        ),
        React.createElement(
          "div",
          ...
          React.createElement("input", {
            ...
            value: minScore,
            onChange: (e) =>
              saveNumber("min_score", e.target.value, 1, 100, setMinScore),
          }),
          React.createElement(
            "small",
            null,
            "Higher values reduce false matches. 58 is a balanced default.",
          ),
        ),
        React.createElement(
          "div",
        ...
        React.createElement(
          ...
          React.createElement(
            "small",
            null,
            "This intercepts ivLyrics translation requests only for this provider and only when translated lyrics were fetched.",
          ),
        ),
        React.createElement(
          "div",
          ...
          React.createElement(
            "small",
            null,
            "Japanese uses Kuroshiro/Kuromoji Hepburn romaji, Korean uses Aromanize RR, and Chinese uses pinyin. NetEase phonetic data is not used.",
          ),
        ),
        React.createElement(
          "div",
          ...
          React.createElement(
            "small",
            null,
            "Use a fixed language if auto detection chooses the wrong converter for mixed-language songs.",
          ),
        ),
        React.createElement(
          "div",
          ...
          React.createElement(
            "small",
            null,
            "Off by default. When off, local failures return empty phonetic lines and do not call AI.",
          ),
        ),
      );
    };
  ...
  const NeteaseLyricsAddon = {
    ...
    async init() {
      ...
      window.__ivLyricsDebugLog?.(
        `[NetEase Lyrics Addon] Initialized (${ADDON_ID})`,
      );
    },
    ...
    async getLyrics(info) {
      ...
      let searchResult;
      try {
        searchResult = await searchNetease(info);
      } catch (error) {
      ...
      let lyricResult;
      try {
        lyricResult = await fetchLyrics(best.song.id);
      } catch (error) {
      ...
      const lyricData = lyricResult?.data || {};
      const lrcRaw = lyricData?.lrc?.lyric || lyricData?.lyric || "";
      if (!String(lrcRaw).trim()) {
      ...
      if (!synced.length) {
        result.unsynced = trimLeadingCreditLines(
          String(lrcRaw)
            .split(/\r?\n/)
        ...
        if (useTranslation) {
          const translatedRaw =
            lyricData?.ytlrc?.lyric ||
            lyricData?.tlyric?.lyric ||
            lyricData?.trans ||
            "";
          const translated = trimLeadingCreditLines(parseLrc(translatedRaw));
      ...
      result.netease = {
        ...
        source: hasProxyUrl()
          ...
          : "public-neteasecloudmusicapi-mirror",
